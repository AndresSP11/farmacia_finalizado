...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $query = "SELECT 
              ...
              FROM 
                medicamento m 
              INNER JOIN 
                petitorio p ON m.codigo_siga = p.codigo_siga
              GROUP BY 
                m.codigo_siga, 
                p.denomin_comun_internac_o_principio_activo
              HAVING 
                p.codigo_siga LIKE CONCAT(?, '%');";
    ...
    $html = "<tr><th>CODIGO</th><th>MEDICAMENTO</th><th>FORMA FARMACEUTICA</th><th>CANTIDAD</th><th>STOCK</th></tr>";

    while ($resultado = $result->fetch_assoc()) {
        $comida = '';
        if ($resultado['total_cantidad'] >= $resultado['minimo_stock']) {
            $comida = 'stock-positivo';
        } elseif ($resultado['minimo_stock'] === NULL) {
            $comida = '';
        } elseif ($resultado['total_cantidad'] < $resultado['minimo_stock']) {
